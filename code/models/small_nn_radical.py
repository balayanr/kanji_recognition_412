...
model.add(Dense(5000, activation='relu', kernel_regularizer=keras.regularizers.l2(0.1)))
model.add(Dense(5000, activation='relu', kernel_regularizer=keras.regularizers.l2(0.1)))
model.add(Dense(num_classes, activation='sigmoid', kernel_regularizer=keras.regularizers.l2(0.1)))
model.compile(optimizer=Adam(lr=0.000001), loss='binary_crossentropy', metrics=['accuracy',multilabel_accuracy])
curr_model = "small_nn_radical_l2_lr=10-6"
